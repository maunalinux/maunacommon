...
def print_timing(func):
    if not DEBUG_MODE:
        ...
        def wrapper(*arg):
            ...
            res = func(*arg)
            t2 = time.time()
            print('mauna-common (DEBUG): %s took %0.3f ms' % (func.__qualname__, (t2 - t1) * 1000.0), flush=True, file=sys.stderr)
            return res
        return wrapper
...
def debug(*args):
    ...
    print("mauna-common (DEBUG): %s" % argstr, file=sys.stderr, flush=True)
...
def warn(*args):
    sanitized = [str(arg) for arg in args if arg is not None]
    argstr = " ".join(sanitized)
    print("mauna-common (WARN): %s" % argstr, file=sys.stderr, flush=True)
